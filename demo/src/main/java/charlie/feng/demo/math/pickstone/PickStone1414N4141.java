...
/**
 * 有两堆石子，分别是1414,4141个，两个人轮流取，
 * 每次可以从一堆中取任意数目的石子，或者从两堆中取同样数目的石子，或相差一个的石子，取最后一个的获胜，
 * 先手和后手谁有必胜策略
 */
...
public class PickStone1414N4141 extends AbstractPickStone2Pile {
    ...
    List<ImmutablePair<Integer, Integer>> getLegalMoves(int i, int j) {
        ...
        IntStream.rangeClosed(1, Math.min(i, j)).forEach(n -> moves.add(ImmutablePair.of(n, n)));
        IntStream.rangeClosed(1, Math.min(i, j - 1)).forEach(n -> moves.add(ImmutablePair.of(n, n + 1)));
        IntStream.rangeClosed(1, Math.min(i - 1, j)).forEach(n -> moves.add(ImmutablePair.of(n + 1, n)));
        return moves;
    }
}
