...
<%@ page import="charlie.feng.game.sudoku.*" %>
		...
		%>
			<TD bgcolor="<%=color %>">
				<%
					...
					for (int k = 0; k<9; k++) {
						if (flag[k].getValue()) {candidateStr += k+1;} else {candidateStr += " ";}
					}
				%>
				<input type="text" name="<%= "cell"+r+c %>" value="<%=sValue%>" size="1" maxlength="1"  onmouseover='showCandidate("<%=candidateStr %>")' onmouseout="clearCandidate()">
			</TD>
		<% } %>
	</TR>
